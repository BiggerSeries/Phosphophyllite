...
public final class JsonLogicParser {
  ...
  private static JsonLogicNode parse(JsonElement root) throws JsonLogicParseException {
    ...
    if (object.entrySet().size() != 1) {
      throw new JsonLogicParseException("objects must have exactly 1 key defined, found " + object.entrySet().size());
    }

    String key = object.entrySet().stream().findAny().get().getKey();
