...
public class MekanismGasWrappers {
    ...
    private static void reloadMappings() {
        ...
        IRecipeType<RotaryRecipe> type = (IRecipeType<RotaryRecipe>) Registry.RECIPE_TYPE.getOrDefault(new ResourceLocation("mekanism:rotary"));
        if (type == null || server == null) {
            return;
        }
        List<RotaryRecipe> recipes = server.getRecipeManager().getRecipesForType(type);
        ...
        for (RotaryRecipe recipe : recipes) {
            ...
            if (recipe.hasGasToFluid()) {
                ...
                FluidStack output = recipe.getFluidOutputRepresentation();
            ...
            if (recipe.hasFluidToGas()) {
                ...
                for (FluidStack input : inputs) {
                    ...
                    long amount = input.getAmount();
                    if (mapping.gasToFluidGasUnits != -1 && mapping.gasToFluidGasUnits != amount) {
                        LOGGER.warn("Input amount discrepancy in rotary recipe " + recipe.getId() + " with fluid " + fluid.getRegistryName() + " wanting " + amount + " input while a different gas wants " + mapping.fluidToGasFluidUnits);
                        continue;
                    }
                ...
                GasStack output = recipe.getGasOutputRepresentation();
